...
class Test(MDGridLayout, MDApp, Widget):
    ...
    def pressed(self, instance):
        top3_rank = final_combiner.final_rank([int(self.left.two.input1.text), int(self.left.two.input3.text), int(self.left.two.input5.text)], self.spinner.text)
        print("實際的前三名：", end="")
        print(top3_rank)
        if not self.acard:
            self.rank1 = ThreeLineIconListItem(text=top3_rank[0][0].split('區')[1], secondary_text='人口特性：' + str(top3_rank[0][1]), tertiary_text='新資所得：' + str(top3_rank[0][2]), text_color=[1,1,1,1])
            ...
            self.rank2 = ThreeLineIconListItem(text=top3_rank[1][0].split('區')[1], secondary_text='人口特性：' + str(top3_rank[1][1]), tertiary_text='新資所得：' + str(top3_rank[1][2]))
            ...
            self.rank3 = ThreeLineIconListItem(text=top3_rank[2][0].split('區')[1], secondary_text='人口特性：' + str(top3_rank[2][1]), tertiary_text='新資所得：' + str(top3_rank[2][2]))
            ...
            self.myLast = MDGridLayout(cols=1, size_hint=[0.8,0.85], md_bg_color=[0.39, 0.4, 0.8, 0.8])
            ...
            lon = 0
            try:
                with open(os.path.realpath(sys.argv[0]).strip(sys.argv[0]) + 'district.json', 'r', encoding='utf-8') as q:
                    q = json.load(q)
